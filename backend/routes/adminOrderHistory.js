...
router.get('/admin-order-history-records', async (req, res) => {
    try {
        ...
        SELECT
            ...
            shipment.address, 
            shipment.city, 
            product.quantity AS product_stock_quantity,
            (product.quantity - order_details.quantity) AS running_balance
        FROM
            \`order\`
        INNER JOIN
            order_details ON \`order\`.order_id = order_details.order_id
        ...
        if (searchTerm) {
            conditions.push(
                `(users.first_name LIKE ? OR users.last_name LIKE ? OR \`order\`.order_id LIKE ?)`
            );
        ...
        const groupedOrders = orders.reduce((acc, order) => {
            ...
            acc[order.order_id].products.push({
                ...
                running_balance: order.running_balance,
                payment_status: order.payment_status,
                payment_method: order.payment_method,
            });
        ...
        if (exportToExcel === 'true') {
            ...
            worksheet.columns = [
                ...
                { header: 'Running Balance', key: 'running_balance', width: 15 },
            ];
            ...
            res.setHeader('Content-Disposition', 'attachment; filename=ordersHistory.xlsx');
            await workbook.xlsx.write(res);
            res.end();
            return;
        }
