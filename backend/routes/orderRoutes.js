...
router.post('/insert-order', async (req, res) => {
    const { customer_id, order_details, total_price } = req.body;

    // Assuming order_date is now set in the frontend
    ...
    if (!customer_id || !order_details || !Array.isArray(order_details) || !total_price) {
        const missingFields = [];
    ...
    try {
        ...
        for (const detail of order_details) {
            const { product_id, quantity, totalprice, payment_method } = detail;

            if (!product_id || !quantity || totalprice == null || !payment_method) {
            ...
            await db.query(`
                INSERT INTO \`order_details\` (order_id, product_id, quantity, total_price, payment_method, order_status, order_date, payment_status) 
                VALUES (?, ?, ?, ?, ?, 'Pending', NOW(), 'Pending')
            `, [order_id, product_id, quantity, totalprice, payment_method]);
        }
